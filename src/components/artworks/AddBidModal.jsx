...
function AddBidModal(props) {
  ...
  return (
    <form
      ...
      className="fixed inset-0 flex justify-center items-center bg-black/[.5] z-20 font-Adamina"
    >
        ...
        <div className="display">
          <div className="artboard artboard-demo phone-1 justify-start">
            ...
            {isLoading ? (
              ...
              <div className="grow w-full px-2 pt-3 flex flex-col gap-3 overflow-y-auto">
                ...
                {highestBid ? (
                  <>
                    <h3>Highest bid</h3>
                    <div className="border border-black/[.3] rounded-md w-full px-2 py-1 flex justify-between items-center bg-black text-white">
                      <div className="flex flex-col gap-2">
                        <h3 className="font-bold">
                          {user.id !== highestBid.user.id
                            ? highestBid.is_anonymous
                              ? 'Anonymous'
                              : highestBid.user.username
                            : 'You'}
                        </h3>
                        <p className="text-xs">
                          {dateFormatter(highestBid.bid_on)}
                        </p>
                      </div>
                ...
                <div>
                  <div class="label">
                    <span class="label-text text-md font-bold">Amount</span>
                  </div>
                  <label className="input input-bordered flex items-center gap-2 rounded-md">
                    ...
                    <input
                      ...
                      className="grow"
                      placeholder={
                        highestBid?.bid_amount || artwork.starting_bid || '0.00'
                      }
                    />
                  </label>
                  {amountErrorMsg && (
                    <p className="text-error text-xs ml-2 mt-2">
                      {amountErrorMsg}
                    </p>
                  )}
                </div>
            ...
            <div className="w-full px-3">
              <button type="submit" className="btn btn-neutral mb-5 w-full">
                Submit
              </button>
            </div>
          </div>
