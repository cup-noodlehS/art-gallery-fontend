...
function EditArtworkPane(params) {
  const { artworkId } = params;
  const router = useRouter();
  const { user, getUser } = useAuthStore();
  const { fetchCategories, createCategory, fetchArtwork, updateArtwork } =
    useArtworkStore();
  ...
  const handleFileChange = (event) => {
    ...
    uploadImage(data)
      .then((res) => {
        ...
        setNewImages([...newImages, res.url]);
  ...
  const updatedImages = prevArtwork
    ? newImages.length > 0 ||
      JSON.stringify(prevArtwork.images.map((image) => image.image_url)) !==
        JSON.stringify(imagesUrl)
    : false;
  const updatedTitle = prevArtwork ? prevArtwork.title !== title : false;
  const updatedDescription = prevArtwork
    ? prevArtwork.description !== description
    : false;
  const updatedStatus = prevArtwork ? prevArtwork.status !== status : false;
  const prevStartingBid =
    prevArtwork && prevArtwork.starting_bid
      ? prevArtwork.starting_bid.toString()
      : null;
  const prevSoldPrice =
    prevArtwork && prevArtwork.sold_price
      ? prevArtwork.sold_price.toString()
      : null;
  const tempPrice = price ? price.toString() : null;
  const updatedPrice = prevArtwork
    ? status === 0
      ? prevStartingBid !== tempPrice
      : prevSoldPrice !== tempPrice
    : false;
  const updatedCategory = prevArtwork
    ? prevArtwork.category?.id !== categoryId
    : false;
  const hasNewChanges =
    updatedImages ||
    updatedTitle ||
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await updateArtwork(artworkId, data);
      setCreatedArtwork(true);
      console.log(res);
      router.push(`/artworks/${res.slug}`);
    } catch (error) {
      console.log(error);
    }
    setIsLoading(false);
  ...
  return (
    <main className="font-Adamina container mx-auto my-5 min-h-lvh">
      ...
      <div className="flex flex-wrap justify-between items-center mb-5 px-3 gap-3">
        ...
        <h1 className="font-bold text-xl sm:text-3xl">Update Artwork</h1>
        <button
          disabled={
            isLoading ||
            !hasNewChanges ||
                  ...
                  <div className="relative">
                    <button className="absolute z-10 translate-x-[-100%] p-1">
                      <FontAwesomeIcon
                        ...
                        className="text-error shadow-md"
                        icon={faTrash}
                        width={20}
                        height={20}
                      />
        ...
        <div className="flex flex-col gap-3 p-5">
          ...
          <label class="form-control w-full">
            ...
            <textarea
              ...
              value={description}
              class="textarea textarea-bordered rounded-sm h-[400px]"
          ...
          <div className="flex gap-3">
            <div className="w-[50%]">
              ...
              <div className="relative">
                {(focusCategorySearch || categorySearchKey.trim().length > 0) &&
                  categoryId === null && (
                      ...
                      {categoryList.map((category) => {
                        return (
                          ...
                          >
                            ...
                            <div className="badge text-white bg-neutral">
                              {limitCategoryCount(category.artwork_count)}
                            </div>
                          </div>
                        );
                      })}
...
export default EditArtworkPane;
