...
function BidsSideBarModal(props) {
    ...
    return (
        <div className="relative">
            <div
                className={`slide-in-bids flex flex-col gap-2 h-full w-[300px] min-w-[300px] translate-x-[-100%] bg-white py-5 px-2 custom-left-shadow absolute
            ...
            >
                ...
                <div className=" grow flex flex-col gap-2 overflow-y-auto">
                    {bids.map((bid, index) => (
                        <div key={bid.id}>
                            <div className="border bg-white border-black/[.3] rounded-md w-full px-2 py-1 flex justify-between items-center">
                                <div className="flex flex-col gap-2">
                                    <Link
                                        href={`/artworks/${bid.artwork.slug}?prev=true`}
                                        className="font-bold hover:underline"
                                    >
                                        {bid.artwork.title}
                                    </Link>
                                    <p className="text-xs">
                                        {dateFormatter(bid.bid_on)}
                                    </p>
                                </div>
