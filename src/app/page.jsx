...
export default function Home() {
  const { fetchArtworks, fetchTopArtist, fetchFeaturedArtworks } =
    useArtworkStore();
        ...
        <div className="container max-w-[1536px] mx-auto">
                ...
                {featuredArtworks.map((artwork, index) => (
                  <div key={index} className="w-full h-full flex flex-col">
                    <div className="relative">
                      <div className="absolute m-5 text-white flex flex-col gap-2">
                        <h1 className="text-2xl font-medium flex items-stretch gap-2">
                          ...
                          <span className="shadow-md-no-off">
                        ...
                        <div className="pl-5 flex flex-col gap-1">
                          ...
                          <p className="shadow-md-no-off">
                            {artwork.artwork.current_highest_bid ? (
                              ...
                              <>
                                Starting Bid: ₱{' '}
                                {artwork.artwork.starting_bid || '0'}
                              </>
                            )}
                          </p>
                          {artwork.artwork.bids_count > 0 && (
                            <p className="shadow-md-no-off">
                              {artwork.artwork.bids_count} Bid
                              {artwork.artwork.bids_count > 1 && 's'}
                            </p>
                          )}
                          <Link
                            href={`/artworks/${artwork.artwork.slug}?prev=true`}
                            className="flex items-center gap-2 btn btn-ghost p-0 rounded-sm w-max shadow-md-no-off"
                          >
                            <svg
                    ...
                    <img
                      src={
                        !isSmallScreen
                          ? artwork.image_url || defaultAvatarUrl
                          : artwork.artwork?.first_image.image_url ||
                            defaultAvatarUrl
                      }
                      alt="image 1"
          ...
          <div className="mt-10 px-3">
            <h2 className="text-4xl font-black mb-3 flex items-stretch gap-2">
              ...
              <span className="mr-2">Top artworks this for sale this week</span>
              <Link
                href="/artworks"
                className="grow whitespace-nowrap text-base self-end text-gray-500 hover:text-sky-300 transition duration-200"
              >
                View all &gt;
              </Link>
            </h2>
            <div className="flex gap-3 pb-3 overflow-x-auto snap-mandatory snap-x overflow-y-hidden">
              {isLoadingArtworks && (
                <>
                  {[...Array(6)].map((_, index) => (
                    <div
                      className="min-w-[350px] max-w-[350px] pb-5"
                      key={index}
                    >
              ...
              {artworks.map((art, index) => (
                <div key={index} className="flex">
                  <div className="h-[500px] overflow-y-hidden">
                    <p className="font-kumar text-[400px] h-max text-gray-700">
                      {index + 1}
                    </p>
                  </div>
          ...
          <div className="mt-10 px-3">
            <h2 className="text-4xl font-black mb-3 flex items-stretch gap-2">
              <div className="w-2 min-w-2 bg-slate-600"></div>
              <span className="mr-2">Popular artists this month</span>
              <Link
                href="/artworks"
                className="grow whitespace-nowrap text-base self-end text-gray-500 hover:text-sky-300 transition duration-200"
              >
                ...
                <div key={index} className="min-w-[300px] pb-5">
                  <div className="relative h-[300px]">
                    <Image
                      ...
                      src={artist.avatar_url || defaultAvatarUrl}
                      alt="Artist Profile"
                      layout="fill"
                      objectFit="cover"
                      className="rounded-full cursor-pointer snap-center"
                    />
                  </div>
